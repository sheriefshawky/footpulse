
/**
 * Translation keys for English and Arabic.
 */
export const translations = {
  en: {
    dashboard: "Dashboard",
    surveys: "Surveys",
    analytics: "Analytics",
    surveyAnalytics: "Survey Analytics",
    questionTrends: "Question Trends",
    users: "Users",
    templates: "Templates",
    manageTemplates: "Manage Templates",
    userManagement: "User Management",
    signOut: "Sign Out",
    academyHub: "Academy Hub",
    performanceOverview: "Performance Overview",
    hello: "Hello",
    trackGrowth: "Track growth metrics below.",
    completed: "Completed",
    avgPerformance: "Avg Performance",
    teamRank: "Team Rank",
    nextSession: "Next Session",
    trainingProgress: "Training Progress",
    actionItems: "Action Items",
    startAssessment: "Start Assessment",
    begin: "Begin",
    registerMember: "Register Member",
    editMember: "Edit Member",
    fullName: "Full Name",
    emailAddress: "Email Address",
    mobileNumber: "Mobile Number",
    personaRole: "Persona Role",
    assignTrainer: "Assign Trainer",
    assignPlayer: "Assign Player",
    createAccount: "Create Account",
    saveChanges: "Save Changes",
    cancel: "Cancel",
    search: "Search players, sessions...",
    evaluationSubmitted: "Evaluation Submitted",
    pendingEvaluation: "Pending Evaluation",
    startReview: "Start Review",
    step: "Step",
    of: "of",
    exitSurvey: "Exit Survey",
    needsImprovement: "Needs Improvement",
    eliteLevel: "Elite Level",
    submitAssessment: "Submit Assessment",
    continue: "Continue",
    playerAssessmentGroup: "Player Assessment Group",
    squadEvaluationCenter: "Squad Evaluation Center",
    loginTitle: "Academy Login",
    loginSubtitle: "Empowering football talent through data-driven training assessments.",
    accessPortal: "ACCESS PORTAL",
    demoPersonas: "Demo Personas",
    coach: "Coach",
    player: "Player",
    guardian: "Guardian",
    admin: "Admin",
    doctor: "Doctor",
    assignPlayers: "Assign Players",
    assignedPlayers: "Assigned Players",
    competencyProfile: "Competency Profile",
    growthPath: "Growth Path",
    dataInsights: "Data Insights",
    squadEvaluationTracker: "Squad Monthly Assessment Tracker",
    password: "Password",
    confirmPassword: "Confirm Password",
    currentPassword: "Current Password",
    newPassword: "New Password",
    changePassword: "Change Password",
    passwordsDoNotMatch: "Passwords do not match",
    profileSettings: "Profile Settings",
    passwordUpdated: "Password updated successfully",
    userUpdated: "User updated successfully",
    addTemplate: "Add Template",
    editTemplate: "Edit Template",
    templateName: "Template Name",
    arabicTemplateName: "Template Name (Arabic)",
    description: "Description",
    arabicDescription: "Description (Arabic)",
    categories: "Categories",
    addCategory: "Add Category",
    categoryName: "Category Name",
    weight: "Weight (%)",
    questions: "Questions",
    addQuestion: "Add Question",
    questionText: "Question Text",
    arabicQuestionText: "Question Text (Arabic)",
    totalWeight: "Total Weight",
    weightWarning: "Total weight should equal 100%",
    delete: "Delete",
    questionType: "Question Type",
    ratingType: "Rating (1-10)",
    choiceType: "Multiple Choice",
    addOption: "Add Option",
    optionLabel: "Choice Label",
    optionLabelAr: "Choice (Arabic)",
    scoreValue: "Score Value",
    selectOption: "Select an option",
    assessmentAbout: "Assessment About",
    position: "Position",
    assignAllGuardians: "Guardians -> Children",
    assignAllGuardiansToCoaches: "Guardians -> Coaches",
    assignAllPlayersToCoaches: "Players -> Coaches",
    assignAllCoachesToPlayers: "Coaches -> Their Players",
    assignAllDoctorsToPlayers: "Doctors -> Their Players",
    bulkActions: "Quick Bulk Assignments",
    manualSelection: "Manual Custom Selection",
    editSurvey: "Edit Survey",
    deleteSurvey: "Delete Survey",
    filterByUser: "Filter by User",
    filterByMonth: "Filter by Month",
    year: "Year",
    week: "Week",
    selectYear: "Select Year",
    selectWeek: "Select Week",
    filterBySurvey: "Filter by Survey",
    filterByCategory: "Filter by Category",
    filterByQuestion: "Filter by Question",
    selectAll: "Select All",
    deselectAll: "Deselect All",
    applyFilters: "Apply Filters",
    clearFilters: "Clear Filters",
    aggregatedScore: "Aggregated Score",
    comparisonView: "Comparison View",
    trendAnalysis: "Trend Analysis",
    selectUser: "Select User",
    selectTemplate: "Select Template",
    noDataForQuestion: "No data available for this question",
    individualQuestionTrends: "Individual Question Trends",
    previousValue: "Previous Value",
    trainingSessions: "Training Sessions",
    addTraining: "Add Training",
    editTraining: "Edit Training",
    trainingDate: "Training Date",
    selectPlayers: "Select Players",
    evaluation: "Evaluation",
    rating: "Rating",
    comments: "Comments",
    saveEvaluation: "Save Evaluation",
    noTrainingSessions: "No training sessions found.",
    overallEvaluation: "Overall Evaluation",
    playerEvaluation: "Player Evaluation",
    recentTrainingEvaluations: "Recent Training Evaluations",
    noRecentEvaluations: "No recent training evaluations found.",
  },
  ar: {
    dashboard: "لوحة التحكم",
    surveys: "الاستبيانات",
    analytics: "التحليلات",
    surveyAnalytics: "تحليلات الاستبيانات",
    questionTrends: "اتجاهات الأسئلة",
    users: "المستخدمين",
    templates: "القوالب",
    manageTemplates: "إدارة القوالب",
    userManagement: "إدارة المستخدمين",
    signOut: "تسجيل الخروج",
    academyHub: "مركز الأكاديمية",
    performanceOverview: "نظرة عامة على الأداء",
    hello: "مرحباً",
    trackGrowth: "تتبع مقاييس النمو أدناه.",
    completed: "مكتمل",
    avgPerformance: "متوسط الأداء",
    teamRank: "ترتيب الفريق",
    nextSession: "الجلسة القادمة",
    trainingProgress: "تقدم التدريب",
    actionItems: "المهام المطلوبة",
    startAssessment: "بدء التقييم",
    begin: "بدء",
    registerMember: "تسجيل عضو",
    editMember: "تعديل عضو",
    fullName: "الاسم الكامل",
    emailAddress: "البريد الإلكتروني",
    mobileNumber: "رقم الجوال",
    personaRole: "الدور",
    assignTrainer: "تعيين مدرب",
    assignPlayer: "تعيين لاعب",
    createAccount: "إنشاء حساب",
    saveChanges: "حفظ التغييرات",
    cancel: "إلغاء",
    search: "بحث عن لاعبين، جلسات...",
    evaluationSubmitted: "تم تقديم التقييم",
    pendingEvaluation: "تقييم معلق",
    startReview: "بدء المراجعة",
    step: "خطوة",
    of: "من",
    exitSurvey: "الخروج من الاستبيان",
    needsImprovement: "بحاجة لتحسين",
    eliteLevel: "مستوى نخبوي",
    submitAssessment: "تقديم التقييم",
    continue: "استمرار",
    playerAssessmentGroup: "مجموعة تقييم اللاعبين",
    squadEvaluationCenter: "مركز تقييم الفريق",
    loginTitle: "تسجيل دخول الأكاديمية",
    loginSubtitle: "تمكين مواهب كرة القدم من خلال تقييمات تدريبية قائمة على البيانات.",
    accessPortal: "دخول البوابة",
    demoPersonas: "نماذج الأدوار",
    coach: "مدرب",
    player: "لاعب",
    guardian: "ولي أمر",
    admin: "مسؤول",
    doctor: "طبيب",
    assignPlayers: "تعيين لاعبين",
    assignedPlayers: "اللاعبين المعينين",
    competencyProfile: "ملف الكفاءة",
    growthPath: "مسار النمو",
    dataInsights: "رؤى البيانات",
    squadEvaluationTracker: "متتبع التقييم الشهري للفريق",
    password: "كلمة المرور",
    confirmPassword: "تأكيد كلمة المرور",
    currentPassword: "كلمة المرور الحالية",
    newPassword: "كلمة المرور الجديدة",
    changePassword: "تغيير كلمة المرور",
    passwordsDoNotMatch: "كلمات المرور غير متطابقة",
    profileSettings: "إعدادات الملف الشخصي",
    passwordUpdated: "تم تحديث كلمة المرور بنجاح",
    userUpdated: "تم تحديث العضو بنجاح",
    addTemplate: "إضافة قالب",
    editTemplate: "تعديل القالب",
    templateName: "اسم القالب",
    arabicTemplateName: "اسم القالب (بالعربي)",
    description: "الوصف",
    arabicDescription: "الوصف (بالعربي)",
    categories: "الفئات",
    addCategory: "إضافة فئة",
    categoryName: "اسم الفئة",
    weight: "الوزن (%)",
    questions: "الأسئلة",
    addQuestion: "إضافة سؤال",
    questionText: "نص السؤال",
    arabicQuestionText: "نص السؤال (بالعربي)",
    totalWeight: "إجمالي الوزن",
    weightWarning: "يجب أن يكون إجمالي الوزن 100%",
    delete: "حذف",
    questionType: "نوع السؤال",
    ratingType: "تقييم (1-10)",
    choiceType: "خيارات متعددة",
    addOption: "إضافة خيار",
    optionLabel: "نص الخيار",
    optionLabelAr: "الخيار (بالعربي)",
    scoreValue: "قيمة الدرجة",
    selectOption: "اختر خياراً",
    assessmentAbout: "تقييم حول",
    position: "المركز",
    assignAllGuardians: "أولياء الأمور -> الأبناء",
    assignAllGuardiansToCoaches: "أولياء الأمور -> المدربين",
    assignAllPlayersToCoaches: "اللاعبين -> المدربين",
    assignAllCoachesToPlayers: "المدربين -> لاعبيهم",
    assignAllDoctorsToPlayers: "الأطباء -> لاعبيهم",
    bulkActions: "تعيينات جماعية سريعة",
    manualSelection: "اختيار مخصص يدوي",
    editSurvey: "تعديل الاستبيان",
    deleteSurvey: "حذف الاستبيان",
    filterByUser: "تصفية حسب المستخدم",
    filterByMonth: "تصفية حسب الشهر",
    year: "السنة",
    week: "الأسبوع",
    selectYear: "اختر السنة",
    selectWeek: "اختر الأسبوع",
    filterBySurvey: "تصفية حسب الاستبيان",
    filterByCategory: "تصفية حسب الفئة",
    filterByQuestion: "تصفية حسب السؤال",
    selectAll: "تحديد الكل",
    deselectAll: "إلغاء تحديد الكل",
    applyFilters: "تطبيق التصفية",
    clearFilters: "مسح التصفية",
    aggregatedScore: "النتيجة الإجمالية",
    comparisonView: "عرض المقارنة",
    trendAnalysis: "تحليل الاتجاهات",
    selectUser: "اختر المستخدم",
    selectTemplate: "اختر القالب",
    noDataForQuestion: "لا توجد بيانات متاحة لهذا السؤال",
    individualQuestionTrends: "اتجاهات الأسئلة الفردية",
    previousValue: "القيمة السابقة",
    trainingSessions: "حصص التدريب",
    addTraining: "إضافة تدريب",
    editTraining: "تعديل التدريب",
    trainingDate: "تاريخ التدريب",
    selectPlayers: "اختر اللاعبين",
    evaluation: "التقييم",
    rating: "التقييم",
    comments: "ملاحظات",
    saveEvaluation: "حفظ التقييم",
    noTrainingSessions: "لا توجد حصص تدريبية.",
    overallEvaluation: "التقييم العام",
    playerEvaluation: "تقييم اللاعب",
    recentTrainingEvaluations: "تقييمات التدريب الأخيرة",
    noRecentEvaluations: "لا توجد تقييمات تدريبية أخيرة.",
  }
};
