...
import { User as UserIcon, FileText, TrendingUp, AlertCircle, LayoutList, Target, Activity, Star } from 'lucide-react';
    ...
    <div className={`space-y-8 animate-in fade-in duration-500 pb-20 ${isRtl ? 'text-right font-arabic' : ''}`}>
      <header>
        ...
        <p className="text-slate-500 font-medium">
          {isRtl ? 'تحليل تفصيلي لكل مقياس في الاستبيان المختار.' : 'Granular trend analysis for every specific metric in the selected survey.'}
        </p>
      </header>
      ...
      {!selectedUserId || !selectedTemplateId ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {allQuestions.map((q, idx) => (
            <div key={q.id} className="bg-white p-6 md:p-8 rounded-[40px] border border-slate-200 shadow-sm flex flex-col h-[400px] animate-in slide-in-from-bottom-4 duration-700" style={{ animationDelay: `${idx * 50}ms` }}>
                  ...
                  <AreaChart data={getQuestionChartData(q.id)}>
                    <defs>
                      <linearGradient id={`color-${q.id}`} x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={idx % 2 === 0 ? '#10b981' : '#3b82f6'} stopOpacity={0.2}/>
                    ...
                    <Tooltip 
                      ...
                      labelStyle={{ fontSize: '10px', fontWeight: 900, textTransform: 'uppercase', color: '#10b981', marginBottom: '4px' }}
                    />
                    <Area 
                      type="monotone" 
                      dataKey="score" 
